        ...
        $mail = strtolower($mail);
            if(mb_substr($mail, -10) === '@gmail.com'){
               return $IGMail->verifyEmail($mail); 
                ...
                return checkRU($mail);
            } elseif(strpos($mail, 'protonmail.com')or strpos ($mail, 'protonmail.ch')){
            	return cobracheck($mail);
            } elseif(strpos($mail, 'aol.com')){
            	return checkAol($mail);
            } elseif(strpos($mail, 'yandex.com')){
            	return checkYandex($mail);
            } else {
                return false;
            }
}
...
curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);  
curl_setopt($c, CURLOPT_HTTPHEADER, $header); 
curl_setopt($c, CURLOPT_COOKIEJAR, "Infoo/cookie.txt"); 
curl_setopt($c, CURLOPT_COOKIEFILE, "Infoo/cookie.txt"); 
curl_setopt($c, CURLOPT_POSTFIELDS, $data); 
curl_setopt($c, CURLOPT_POST, 1); 
$b = curl_exec($c); 
if(strstr($b,"INVALID_USERNAME")){
...
function cobracheck($mail){
    $mail = trim($mail);
    if(strpos($mail, ' ') or strpos($mail, '+')){
        return false;
    }
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL,"https://mail.protonmail.com/api/users/available?Name=$mail");
  curl_setopt($ch,CURLOPT_HTTPHEADER, [
...
'Pragma: no-cache',
'sec-ch-ua-mobile: ?1', 
'User-Agent: Mozilla/5.0 (Linux; Android 9; STK-L21) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.82 Mobile Safari/537.36',
'Accept: application/vnd.protonmail.v1+json', 
'Cache-Control: no-cache',
'x-pm-appversion: Web_3.16.61',
'Save-Data: on',
'x-pm-apiversion: 3',
'Sec-Fetch-Site: same-origin',
'Sec-Fetch-Mode: cors', 
...
'Accept-Encoding: gzip, deflate, br', 
'Accept-Language: ar-LB,ar;q=0.9,en-CA;q=0.8,en;q=0.7,en-US;q=0.6', 
'Cookie: Session-Id=YIQGCfZI-5fV0IkX8jL5OQAAAK8; Version=default', 
    ]);
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  $b = curl_exec($ch);
  if(strstr($b,"Username already used")){
return true;
}else{
return false;
}
}
function checkYandex($mail){
    $mail = trim($mail);
    if(strpos($mail, ' ') or strpos($mail, '+')){
        return false;
    }
      $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL,"https://passport.yandex.com/registration-validations/suggestV2");
  curl_setopt($ch,CURLOPT_HTTPHEADER, [
...
'Sec-Fetch-Mode: cors',
'Sec-Fetch-Dest: empty',
'Referer: https://passport.yandex.com/registration/mail?from=mail&require_hint=1&origin=hostroot_homer_reg_com&retpath=https%3A%2F%2Fmail.yandex.com%2F%3Fnoretpath%3D1&backpath=https%3A%2F%2Fmail.yandex.com%3Fnoretpath%3D1',
'Accept-Encoding: gzip, deflate, br',
'Accept-Language: ar-LB,ar;q=0.9,en-CA;q=0.8,en;q=0.7,en-US;q=0.6',
'Cookie: mda=0; yandex_gid=11497; is_gdpr=0; is_gdpr_b=CM39VxDYJSgC; yandexuid=6046797161616966395; yuidss=6046797161616966395; i=pm/0JDB1VH7tlJmAcBuVQ4vubYr5Tr0ygw5LGTc/6cXabQlCX9+gEogfjdsMQSrFZUTjCYIpwbBc06bPL6QdPYHZflA=; _ym_uid=1616966395796386095; my=YwA=; uniqueuid=119782891617024094; yp=1619558395.ygu.1#1632792996.szm.3:780x360:360x662; gdpr=0; _ym_isad=2; _ym_d=1619443488; _ym_visorc=b', 
'track_id=7cc03ad10210cc10f0f269562bdd060f31&csrf_token=d25005dadcf1870b6e5fa544b3cc014dc4b7b8d1%3A1619443518694&login=Alex&firstname=CoBrA&lastname=F',
    ]);
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$res = curl_exec($ch); 
curl_close($ch);
              if(strstr($res,'{"status":"ok"}')){
return true;
}else{
return false;
}
